...
function getDigitalRoot(num) {
  let sum = num.toString().split('').reduce((acc, val) => acc + +val, 0);
  while (sum > 9) sum = sum.toString().split('').reduce((acc, val) => acc + +val, 0);
  return sum;
}
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (map[str[i]] === str[i] && arr[arr.length - 1] === str[i]) arr.pop();
    else if (map[str[i]]) arr.push(str[i]);
    else if (map[arr.pop()] !== str[i]) return false;
  }
